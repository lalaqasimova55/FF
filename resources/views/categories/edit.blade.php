...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Kateqoriya Düzəliş Et</h1>
...
<div class="bg-white rounded shadow p-6 max-w-2xl">
    <form method="POST" action="{{ route('categories.update', $category) }}">
                    ...
                    <label class="flex items-center cursor-pointer">
                        ...
                        <div class="w-8 h-8 rounded border-2"
                            @style([
                                ...
                                'border-color: ' . (old('color', $category->color) == $color ? '#000' : '#ddd'),
                            ])></div>
                    </label>
                @endforeach
            </div>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('categories.index') }}" class="bg-gray-200 text-gray-700 px-6 py-2 rounded hover:bg-gray-300">
                Ləğv et
            </a>
        </div>
    </form>
